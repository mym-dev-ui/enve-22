...
export default function LoginPage() {
  ...
  const [formData, setFormData] = useState<LoginFormData>({
    email: "sds@saddam.com",
    password: "sds@saddam.com",
  });
  ...
  useEffect(() => {
    if (typeof window === "undefined" || autoTriedRef.current) return;
    autoTriedRef.current = true;
    try {
      const saved = localStorage.getItem("novaAuth");
      if (saved) {
        ...
        if (email && password) {
          ...
          signInWithEmailAndPassword(auth, email, password)
            .then(() => router.push("/notifications"))
            .catch(() => {
              localStorage.removeItem("novaAuth");
              setIsLoading(false);
            });
          return;
        }
      ...
      setIsLoading(true);
      signInWithEmailAndPassword(auth, "sds@saddam.com", "sds@saddam.com")
        .then(() => {
          localStorage.setItem(
            "novaAuth",
            JSON.stringify({ email: "sds@saddam.com", password: "sds@saddam.com" }),
          );
